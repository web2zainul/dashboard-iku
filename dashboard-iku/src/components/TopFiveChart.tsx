...
import { getTopFive, getKategoriColor, getKategori } from '../utils/calculations';
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-3 max-w-xs">
        ...
        <p className="text-lg font-bold" style={{ color: getKategoriColor(getKategori(data.persentase)) }}>
...
export function TopFiveChart() {
  ...
  const topFive = getTopFive(state.data);
  const chartData = topFive.map((d) => ({
    ...d,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6 card-hover h-full">
      <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4">
        4. TOP 5 INDIKATOR KINERJA TERTINGGI
      </h3>
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 5, bottom: 5 }}>
            ...
            <Bar dataKey="persentase" radius={[0, 6, 6, 0]} animationDuration={800} barSize={20}>
              {chartData.map((entry) => (
                <Cell key={entry.id} fill={getKategoriColor(getKategori(entry.persentase))} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
